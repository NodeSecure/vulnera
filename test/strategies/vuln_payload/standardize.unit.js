...
test("should convert NodeWG strategy payload into NodeSecure standard payload", (tape) => {
  const [nodeWGToStandardFormat] = formatVulnerabilities(
    VULN_MODE.SECURITY_WG,
    SECURITYWG_VULNS_PAYLOADS.inputVulnsPayload.vulnerabilities
  );
  tape.deepEqual(
    nodeWGToStandardFormat,
    SECURITYWG_VULNS_PAYLOADS.outputStandardizedPayload
  );
  tape.end();
});
